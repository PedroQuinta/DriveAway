...
<head>
	...
	<style type="text/css">
		...
		@media screen and (max-width: 1300px) {
           	#vehicle_info{margin-left: 0px; margin-right: 0px; float: right; width: 750px;}
           	#vehicle_content{width: 200px;}   
        }
	</style>
</head>
<body>
	<div id="add_record" class="row row-content align-items-left">
		<a href="#" data-url="Gestao/register_vehicle.php" class="btn btn-success pull-left">Adicionar Veiculo</a>
	</div>	
					...
					if($stmt->execute()){
						if($stmt->rowCount() > 0){
							while ($row_vehicle = $stmt->fetch()) {
								echo "<tr>";	
									echo "<td><a href='#' data-url='".$gestao."editar_veiculo.php?idveiculo=".$row_vehicle['idveiculo']."'>".$row_vehicle['marca']."-".$row_vehicle['modelo']."</a></td>";
								echo "</tr>";
							}
						}
					}
	...
	<script type="text/javascript">		
		...
		$(document).on("click", "#atualiza_veiculo", function(){				
			if($("#marca").val()!="" && $("#modelo").val()!="" && $("#matricula").val()!=""){
				$.ajax({
				  type: "POST",
				  url: "Gestao/editar_veiculo.php?teste=1&idveiculo="+$("#hidden_id_vehicle").val(),
